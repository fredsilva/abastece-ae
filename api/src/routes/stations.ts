...
export const stations = new Hono<{ Bindings: Env }>();
...
stations.get("/", async (c) => {
  ...
  const cityId = c.req.query("city") || null;
  ...
  const query = cityId
    ? c.env.DB.prepare(
        `SELECT gs.id, gs.nome_fantasia, gs.brand, gs.address_street, gs.address_number, gs.address_neighborhood,
                gs.city, gs.state, gs.city_id, gs.latitude, gs.longitude,
         ...
         WHERE gs.status = 'active' AND gs.city_id = ?
  ...
  const { results } = await query.all<StationPriceRow>();
  ...
  const stationsOut = ranked.map((r) => {
  ...
  return c.json({ fuel, city: cityId, stations: stationsOut });
});
