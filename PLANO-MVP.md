...
- **Painel admin:** mantém a paleta quase monocromática original do Cal.com (preto como cor de ação dominante); badges pastéis reservados para tags secundárias (ex. bandeira do posto), nunca nos CTAs principais.
- **App mobile (decisão da Etapa 3):** diverge do monocromático do site institucional — `{colors.brand-accent}` (#3b82f6) é a cor de ação dominante (abas ativas, wordmark "Aê", CTAs primários, pin do usuário no mapa, badge "Definir como aba padrão"), no lugar do preto. Motivo: o mockup de referência do app usa azul como identidade visual própria do produto mobile, diferente da superfície de marketing/admin. Preto (`{colors.ink}`) permanece só para texto. `{colors.success}` (#10b981) é usado para o badge/realce "Mais barato" (mais forte que o `{colors.badge-orange}` reservado agora só pro badge "Desconto no Pix ou Dinheiro" e para `{component.rating-stars}`). Ícones de posto usam um pequeno rodízio de cores pastel (`success`, `brand-accent`, `badge-violet`, `badge-orange`) por hash do id, sem significado semântico. Modo escuro está travado (`userInterfaceStyle: "light"`) — segue fora do escopo do MVP, e o app não tem uma variante escura dos tokens.
...
A ANP publica (1) registro de revendedores autorizados (CNPJ, endereço, bandeira — sem lat/lng) e (2) pesquisa semanal de preços de combustíveis. **Achado da implementação:** ao contrário do que se assumia no desenho original, a pesquisa semanal (2) traz preço **por posto individual** (nome, CNPJ, endereço, produto, valor, data da coleta), não só a média por município — mas cobre apenas uma amostra dos postos cadastrados, não todos. Na prática: postos que caem na amostra da semana recebem preço real (com `confidence_score=0.6`, já que é dado oficial mas ainda não verificado pela comunidade); os demais recebem a média municipal calculada a partir da própria amostra, como estimativa (`confidence_score=0`, rotulado na UI como "aguardando confirmação"). Fontes reais confirmadas: cadastro em `gov.br/anp/.../dados-cadastrais-dos-revendedores-varejistas-de-combustiveis-automotivos` (CSV estável, `;`-delimitado, UTF-8) e pesquisa semanal em `gov.br/anp/.../serie-historica-de-precos-de-combustiveis` (arquivos mensais por combustível, nomeação inconsistente mês a mês — baixar manualmente o mais recente).
